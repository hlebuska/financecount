...
describe('TransactionCategorizerService', () => {
  ...
  describe('adversarial scenarios', () => {
    ...
    it('does not persist enrichment when the normalized transaction has no merchant candidate', async () => {
      const prisma = {
        category: {
          findUnique: jest.fn(),
          findMany: jest.fn(),
      ...
      const result = await service.categorize({
        userId: 'user-1',
        rawExtractedTransactionId: 'raw-no-merchant',
      ...
      expect(merchantEnrichmentService.enrich).not.toHaveBeenCalled();
      expect(prisma.merchantEnrichmentResult.upsert).not.toHaveBeenCalled();
      expect(result).toEqual({
